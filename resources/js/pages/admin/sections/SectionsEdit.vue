...
export default {
    ...
    methods: {
        cancel () {
            window.events.$emit('sections:edit-cancel')
        ...
        async update () {
            let { data } = await axios.put(`${this.urlBase}/api/admin/sections/${this.section.id}`, this.form)
    ...
    async mounted () {
        this.form.name_en = this.section.name_en
        this.form.name_fr = this.section.name_fr
    }
}
</script>